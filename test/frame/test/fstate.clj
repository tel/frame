...
(deftest test-promote-sfn
  (testing "Should not perturb normal values"
    (is (= (eval-state (promote-sfn 3)) 3))
    (is (= (eval-state (promote-sfn [3])) [3]))
    (is (= (eval-state (promote-sfn (list 3))) (list 3)))
    (is (= (eval-state (promote-sfn {:a 3})) {:a 3})))
  (testing "Should promote stateful values"
    (let [comp (with-monad state-m (fetch-state))] 
      (is (= (eval-state (promote-sfn comp)) {}))
      (is (= (eval-state (promote-sfn [comp])) [{}]))
      (is (= (eval-state (promote-sfn (list comp))) (list {})))
      (is (= (eval-state (promote-sfn {:a comp})) {:a {}}))))
  (testing "Should promote stateful values from deep structures"
    (let [comp (with-monad state-m (fetch-state))
          embed (fn [x] [1 2 3 [4 5 6 [7 8 [9 x]]]])] 
      (is (= (eval-state (promote-sfn (embed comp))) (embed {}))))))
